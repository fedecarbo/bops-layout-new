...
category: Forms
element: Textareas
variant: With title and pill actions
tech: React
tailwind: v4.1
deps:
  - '@headlessui/react'
...
tags:
  ...
  - form-field
a11y:
  - sr-only labels for title and description
  - Listbox components provide ARIA semantics
  - sr-only label on attachment button
  - aria-hidden on decorative spacers
notes: |
  Combined title input and description textarea with metadata actions. Uses absolute
  positioning for bottom toolbar with pill-shaped Listbox buttons (assignee, label,
  due date). Spacer elements maintain proper textarea height with toolbar overlay.
---
