...
category: Forms
element: Form Layouts
variant: Labels on left
tech: React
tailwind: v4.1
deps:
...
tags:
  ...
  - responsive
a11y:
  - labeled inputs with htmlFor association
  - aria-describedby for checkbox descriptions
  - sr-only legends with visible aria-hidden headings
  - aria-hidden on decorative icons
notes: |
  Horizontal label-left layout using sm:grid-cols-3 where labels are in column 1 and inputs span columns 2-3.
  Uses divide-y utilities with border-t/border-b for row separators creating a clean divided appearance.
  Field inputs have constrained max-widths (max-w-xs, max-w-md, max-w-xl, max-w-2xl) based on expected content length.
---
