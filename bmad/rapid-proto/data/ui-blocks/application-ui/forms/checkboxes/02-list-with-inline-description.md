...
category: Forms
element: Checkboxes
variant: List with inline description
tech: React
tailwind: v4.1
deps: []
tags:
  ...
  - svg-icon
a11y:
  - fieldset with sr-only legend
  - aria-describedby linking description to input
  - associated labels with htmlFor
  - sr-only text for screen reader context
  - keyboard navigation support
notes: |
  Similar to list with description but description appears inline on same line as label.
  Uses sr-only spans to provide additional context for screen readers. Same grid overlay
  technique for custom checkbox with SVG checkmark and indeterminate state support.
---
