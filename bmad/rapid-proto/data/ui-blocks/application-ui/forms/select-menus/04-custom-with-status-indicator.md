...
variant: Custom with status indicator
tech: React
tailwind: v4.1
deps:
  - '@headlessui/react'
...
tags:
  ...
  - form-field
a11y:
  - Listbox component provides proper ARIA semantics
  - aria-label on status indicator
  - sr-only text provides online/offline state
notes: |
  Custom select with colored status indicator dots. Uses classNames helper for
  conditional styling. Status dots have forced-colors support for high contrast mode.
---
