...
notes: |
  Full-page week calendar with hourly time slots and positioned events.
  Uses CSS Grid with inline styles for precise event positioning based on time (gridRow for start/duration).
  Horizontal scrolling enabled on mobile with wider grid (165% width) to accommodate all days.
  Events use color-coded backgrounds with before pseudo-elements for layered appearance.
  Time labels positioned with sticky positioning and negative margins for alignment with grid.
  Responsive: compact mobile view with abbreviated day headers, full desktop view with expanded labels.
---
