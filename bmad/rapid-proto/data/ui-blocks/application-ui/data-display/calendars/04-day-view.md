...
tags:
  ...
  - responsive
a11y:
  - sr-only labels for navigation
  - aria-hidden on decorative icons
  - semantic time elements
  - menu pattern with HeadlessUI
  - keyboard focus management
notes: |
  Full-page single-day calendar with hourly time slots and optional month sidebar.
  Uses CSS Grid with inline styles for precise event positioning based on time (gridRow for start/duration).
  Desktop shows split view with day schedule on left and month calendar on right sidebar.
  Events use color-coded backgrounds with before pseudo-elements for layered appearance.
  Mobile displays mini week selector above day grid for quick navigation.
  Time labels positioned with negative margins for alignment with grid.
  Responsive: full-width on mobile, two-column layout (schedule + sidebar) on md+ breakpoints.
---
