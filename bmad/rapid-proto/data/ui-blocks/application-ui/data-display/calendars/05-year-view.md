...
notes: |
  Full-year calendar grid displaying all 12 months at once.
  Uses responsive grid that adjusts from 1 column (mobile) to 4 columns (2xl) breakpoints.
  Each month is a self-contained section with gap-px grid for visual date separation.
  Data attributes control today highlighting and current month styling.
  Header with view switcher dropdown and year navigation controls.
  Responsive grid: 1 column → 2 columns → 3 columns → 4 columns across breakpoints.
---
