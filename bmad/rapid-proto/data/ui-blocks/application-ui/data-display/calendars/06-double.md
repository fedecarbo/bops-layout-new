...
notes: |
  Two-month calendar view with upcoming events list below.
  Uses responsive grid showing two months side-by-side on md+ breakpoints, single month on mobile.
  Second month hidden on mobile with last:max-md:hidden utility.
  Navigation arrows positioned absolutely in top corners.
  Events list uses flexible layout with date, event name, and time range.
  Data attributes control today highlighting and current month styling.
  Responsive: 1 month on mobile, 2 months side-by-side on md+ breakpoints.
---
