...
a11y:
  ...
  - sr-only context for trend changes
  - sr-only context for view all links
notes: |
  Feature-rich stat cards with brand icons, trend indicators, and action links.
  Brand icons use absolute positioning with indigo-500 background for visual hierarchy.
  Trend indicators show directional arrows with conditional colors (green for increase, red for decrease).
  Footer links positioned absolutely at card bottom with semi-transparent background overlay.
  Responsive grid: 1 column on mobile, 2 on sm, 3 on lg breakpoints.
---
