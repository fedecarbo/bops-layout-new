...
a11y:
  ...
  - colSpan attribute for spanning group headers
  - sr-only labels for action columns and edit links
notes: |
  Table with grouped rows organized by location/category using Fragment wrapper.
  Group headers use scope="colgroup" with colSpan={5} and semi-transparent background.
  Conditional borders: first row in each group has heavier border (border-white/15), others lighter (border-white/10).
  Uses classNames helper for dynamic border styling based on row position.
---
