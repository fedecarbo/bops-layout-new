...
category: Lists
element: Tables
variant: With stacked columns on mobile
tech: React
tailwind: v4.1
deps: []
tags:
...
a11y:
  ...
  - sr-only labels for action columns and stacked data terms
  - description list (dl/dt/dd) for stacked mobile content
  - sr-only context for edit links
notes: |
  Responsive table that stacks columns under the Name cell on mobile/tablet.
  Uses description list (dl/dt/dd) with sr-only labels for stacked content.
  Progressive disclosure: Title hidden <lg, Email hidden <sm. No horizontal scroll.
  First cell uses w-full/max-w-0 on mobile, auto-sizing on larger screens.
---
