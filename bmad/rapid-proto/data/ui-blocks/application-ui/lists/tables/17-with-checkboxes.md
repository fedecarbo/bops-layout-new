...
a11y:
  ...
  - checkbox inputs with proper checked/indeterminate states
  - sr-only labels for action columns and edit links
  - focus-visible styles for keyboard navigation
notes: |
  Interactive table with row selection via checkboxes and bulk action toolbar.
  Uses React hooks (useState, useLayoutEffect, useRef) for selection state management.
  Select-all checkbox supports indeterminate state when some rows selected.
  Bulk actions toolbar appears when selections exist using group-has-checked/table:flex.
  Selected rows show indigo accent color, left border indicator, and background highlight.
  Custom checkbox styling with grid-based SVG icon overlay for check/indeterminate marks.
---
