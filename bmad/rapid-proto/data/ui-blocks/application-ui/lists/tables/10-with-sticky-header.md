...
a11y:
  ...
  - sr-only labels for action columns
  - sr-only context for edit links
  - hidden columns revealed progressively via breakpoints
notes: |
  Table with sticky header that remains visible during scroll using position: sticky.
  Uses border-separate with border-spacing-0 for proper border rendering with sticky positioning.
  Header has semi-transparent background (bg-gray-900/75) with backdrop-blur for frosted glass effect.
  Conditional borders on tbody cells except last row using classNames helper.
---
