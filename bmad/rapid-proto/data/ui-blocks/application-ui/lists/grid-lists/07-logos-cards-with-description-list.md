...
category: Lists
element: Grid Lists
variant: Logos cards with description list
tech: React
tailwind: v4.1
deps:
  - '@headlessui/react'
...
a11y:
  ...
  - sr-only menu action context
  - semantic time element with dateTime
  - definition list for key-value pairs
notes: |
  Client invoice cards with logo headers and description list content.
  Uses Headless UI Menu component for actions dropdown with transition animations.
  Conditional badge rendering based on invoice status (Paid, Withdraw, Overdue) with appropriate colors.
---
