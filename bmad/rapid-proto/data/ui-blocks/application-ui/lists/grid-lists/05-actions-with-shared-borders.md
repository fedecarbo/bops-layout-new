...
a11y:
  ...
  - absolute positioned link for full card clickability
  - aria-hidden on decorative icons and arrow
  - sr-only link context extensions
notes: |
  Two-column grid with shared borders between cards using divide utilities and conditional border classes.
  Complex border logic handles corner rounding based on position (first, last, odd, even).
  Colored icon badges with dynamic foreground/background colors and decorative arrow on hover.
---
