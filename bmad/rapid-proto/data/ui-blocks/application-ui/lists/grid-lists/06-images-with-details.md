...
a11y:
  ...
  - sr-only button labels with file context
  - focus-within outline for keyboard navigation
  - pointer-events-none on text to prevent interaction conflicts
notes: |
  Image gallery grid with responsive columns (2 → 3 → 4) and varying gap sizes.
  Uses aspect-10/7 for consistent image ratios and group-hover opacity effect.
  Button overlays entire image with sr-only label for accessibility.
---
