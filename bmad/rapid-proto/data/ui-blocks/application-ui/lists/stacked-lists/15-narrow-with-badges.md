...
a11y:
  ...
  - heading element (h2) for deployment names
  - absolute positioning for full-row click target
  - aria-hidden="true" on decorative chevron icon
notes: |
  Deployment list with colored status indicators (offline, online, error) and environment badges.
  Uses conditional rendering for status dots and badges (Preview, Production).
  Clickable rows with absolute positioning and chevron indicator.
---
