...
variant: With badges, button, and actions menu
tech: React
tailwind: v4.1
deps:
  - '@headlessui/react'
...
a11y:
  ...
  - sr-only labels for menu button and context-specific actions
  - aria-hidden="true" on decorative ellipsis icon
  - time element with datetime attribute
  - menu pattern via Headless UI
notes: |
  Project list with colored status badges (In progress, Complete, Archived).
  Features a "View project" button (hidden on mobile) and actions dropdown menu.
  Uses conditional rendering for badge colors based on status values.
---
