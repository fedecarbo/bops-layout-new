...
category: Navigation
element: Tabs
variant: Bar with underline
tech: React
tailwind: v4.1
deps:
  - '@heroicons/react/16/solid'
tags: [navigation, tabs, bar, responsive]
a11y:
  - "Uses <nav aria-label='Tabs'>"
  - "aria-current for active tab"
  - "aria-label on select element"
  - "aria-hidden on chevron icon and underline indicator"
notes: |
  Tabs in a contained bar with dividers and rounded corners.
  Active tab shows indigo underline indicator at bottom.
  Uses isolate for z-index stacking context.
  Replace indigo brand colors as needed.
---
