...
category: Navigation
element: Navbars
variant: Dark with centered search and secondary links
tech: React
tailwind: v4.1
deps:
  - '@headlessui/react'
...
notes: |
  Two-row dark navbar with centered search and secondary navigation row. Desktop layout uses divide-y for visual separation between header row and navigation row. Search absolutely positioned and centered on desktop (sm:absolute sm:inset-0), inline centered on mobile. Secondary navigation hidden on mobile, shown in separate row below on desktop with rounded button styling.
---
