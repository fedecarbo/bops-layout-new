...
a11y:
  ...
  - aria-hidden on decorative indicators and separators
notes: |
  Dark paneled progress with detailed step descriptions and responsive border layout.
  Mobile uses vertical stacking with individual borders, desktop uses horizontal flex with full-width borders.
  Active indicator bar on left (mobile) switches to bottom (desktop), lg:pl-9 indents content after first step.
  Complete steps show CheckIcon, current shows ID in bordered circle, upcoming shows ID in gray.
  SVG chevron separators between steps on desktop only, hover effects on complete/upcoming steps.
---
