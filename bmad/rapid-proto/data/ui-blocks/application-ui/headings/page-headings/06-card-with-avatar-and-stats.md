...
notes: |
  Card-based profile heading with avatar, welcome message, and stats grid.
  Two-tone background uses layered bg-gray utilities for depth.
  Stats section uses responsive grid that switches from stacked (divide-y) to
  horizontal (divide-x) layout at sm breakpoint. SR-only heading provides
