...
notes: |
  Card heading with descriptive text and primary action button.
  Two-line heading pattern with title and supporting description below.
  Uses negative margin technique (-mt-4 -ml-4) with positive margins on children
  for consistent spacing. Responsive layout wraps on mobile (flex-wrap) and
  stays inline on desktop (flex-nowrap).
---
