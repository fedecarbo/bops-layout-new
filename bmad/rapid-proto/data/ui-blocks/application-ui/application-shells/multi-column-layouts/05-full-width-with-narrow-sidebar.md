...
variant: Full-width with narrow sidebar
tech: React
tailwind: v4.1
deps:
  - '@headlessui/react'
...
notes: |
  Three-column layout with narrow icon-only sidebar (w-20), secondary column (w-96), and main content.
  Desktop sidebar uses before pseudo-element for border and background, centered icon navigation.
  Icon-only navigation items use sr-only labels for accessibility.
  Secondary column fixed at left-20 offset, main content has xl:pl-96 to accommodate both columns.
  Mobile sidebar expands to full width with text labels.
---
