...
variant: Constrained with sticky columns
tech: React
tailwind: v4.1
deps:
...
notes: |
  Constrained three-column layout with sticky positioned side columns (top-8).
  Left column (w-44, visible lg+) and right column (w-96, visible xl+) remain in viewport on scroll.
  Uses gap-x-8 for column spacing instead of borders, creating cleaner visual separation.
  Header has border-b directly on element with before pseudo-element for background overlay.
  Main content area is flex-1 allowing it to grow and fill available space.
---
